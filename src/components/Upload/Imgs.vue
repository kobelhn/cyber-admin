...
// TODO: 图片上传目前走的接口，接口将文件上传到了本地目录，后续可以改造为通过接口上传到云服务器
...
const uploadSuccess = (response: { fileUrl?: string; filename?: string } | undefined, uploadFile: UploadFile) => {
  if (!response) return;
  uploadFile.url = response.fileUrl || response.filename;
