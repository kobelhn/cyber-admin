...
/**
 * @description 使用递归过滤出需要渲染在左侧菜单的列表 (需剔除 show === 0 的菜单)
 * @param {Array} menuList 菜单列表
 * @returns {Array}
 * */
export function getShowMenuList(menuList: Menu.MenuOptions[]) {
  let newMenuList: Menu.MenuOptions[] = JSON.parse(JSON.stringify(menuList));
  const arr = newMenuList.filter(item => {
    item.children?.length && (item.children = getShowMenuList(item.children));
    return item.meta?.show === 1;
  });
  return arr;
}
...
export function generateDebounceFn<T extends (...args: any[]) => void>(fn: T, delay: number): (...args: Parameters<T>) => void {
